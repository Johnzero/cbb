...
function(a, b) {
        ...
        if (Y.isBlob(a) && d(f)) {
            ...
            T(h, M, function j(b) {
                ...
                "progress" == d ? c(a, e, b, b.target.result, {
                    loaded: b.loaded,
                    total: b.total
                }) : "loadend" == d ? (U(h, M, j), h = null) : c(a, e, b, b.target.result)
            });
            try {
    ...
    function f(a, b) {
        if (!a.type && a.size % 4096 === 0 && a.size <= 102400) if (u) try {
            var c = new u;
            V(c, M, function(a) {
                var d = "error" != a.type;
                b(d), d && c.abort()
    ...
    function h(a, b) {
        if (a) if (a.isFile) a.file(function(c) {
    ...
    function l(a) {
        var b;
        for (b in a) if (a.hasOwnProperty(b) && !(a[b] instanceof Object || "overlay" === b || "filter" === b)) return !0;
        return !1
    }
    var m = 1,
        n = function() {}, o = a.document,
        p = o.doctype || {}, q = a.navigator.userAgent,
        ...
        Q = function(a) {
            ...
            if (a) if (Q(a)) for (var d = 0, e = a.length; e > d; d++) d in a && b.call(c, a[d], d, a);
            else for (var f in a) a.hasOwnProperty(f) && b.call(c, a[f], f, a)
        }, S = function(a) {
            ...
            if (a) {
                var d = Y.uid(a),
                    e = K[d] || {}, f = u && a && a instanceof u;
                R(b.split(/\s+/), function(b) {
                    if (x && !f) x.event.remove(a, b, c);
                    else {
                        for (var d = e[b] || [], g = d.length; g--;) if (d[g] === c) {
                            d.splice(g, 1);
                            break
                        }
            ...
            readAsImage: function(a, d, e) {
                ...
                else if (C.test(a.nodeName)) if (a.complete) c(a, d, "load", a);
                else {
                    var g = "error abort load";
                    V(a, g, function i(b) {
                        "load" == b.type && r && r.revokeObjectURL(a.src), U(a, g, i), c(a, d, b, a)
                    })
            ...
            getDropFiles: function(a, b) {
                ...
                R((e ? d.items : d.files) || [], function(a) {
                    ...
                    try {
                        ...
                        i.next(), Y.log("[err] getDropFiles: ", b)
                    }
                }), i.check()
            },
            ...
            filterFiles: function(a, b, c) {
                if (a.length) {
                    var d, e = a.concat(),
                        ...
                        g = [];
                    ! function h() {
                        e.length ? (d = e.shift(), Y.getInfo(d, function(a, c) {
            ...
            _getFormData: function(a, b, c) {
                var d = b.file,
                    ...
                    k = h && l(h),
                    m = Y.postNameConcat;
                R(a.data, function n(a, b) {
                    "object" == typeof a ? R(a, function(a, c) {
                        n(a, m(b, c))
                    }) : i.append(b, a)
                }),
                function o(b) {
                    ...
                    })) : Y.Image && h && (/^image/.test(b.type) || E.test(b.nodeName)) ? (j.inc(), k && (h = [h]), Y.Image.transform(b, h, a.imageAutoOrientation, function(c, d) {
                        if (k && !c) B || Y.flashEngine || (i.multipart = !0), i.append(e, d[0], f, h[0].type || g);
                        else {
                            var l = 0;
                            c || R(d, function(a, b) {
    ...
    Y.addInfoReader(/^image/, function(a, b) {
        ...
        c || (c = b, b = Y.F), u ? (T(a, "dragenter dragleave dragover", b.ff = b.ff || function(a) {
            for (var c = k(a).types, f = c && c.length, g = !1; f--;) if (~c[f].indexOf("File")) {
                ...
                if (g = h.xhr = b.getXHR(), e.params && (k += (k.indexOf("?") < 0 ? "?" : "&") + e.params.join("&")), g.open("POST", k, !0), b.withCredentials && (g.withCredentials = "true"), c.headers && c.headers["X-Requested-With"] || g.setRequestHeader("X-Requested-With", "XMLHttpRequest"), b.each(c.headers, function(a, b) {
                    ...
                    g.upload && g.upload.addEventListener("progress", b.throttle(function(a) {
                        ...
                        if (h.status = g.status, h.statusText = g.statusText, h.readyState = g.readyState, 4 == g.readyState) {
                            ...
                            if (g.onreadystatechange = null, !g.status || g.status - 201 > 0) if (b.log("Error: " + g.status), (!g.status && !g.aborted || 500 == g.status || 416 == g.status) && ++e.retry <= c.chunkUploadRetry) {
                    ...
                    if (h.status = g.status, h.statusText = g.statusText, h.readyState = g.readyState, 4 == g.readyState) {
                        ...
                        if (g.onreadystatechange = null, !g.status || g.status > 201) if (b.log("Error: " + g.status), (!g.status && !g.aborted || 500 == g.status) && (c.retry || 0) < c.uploadRetry) {
                            c.retry = (c.retry || 0) + 1;
                    ...
                    c.flashEngine = !0, j(c, {
                        ...
                        getInfo: function(a, b) {
                            if (k(a)) this.parent.apply(this, arguments);
                                ...
                                if (!a.__info) {
                                    var d = a.__info = c.defer(); /*v.cmd(a,"getFileInfo",{id:a.id,callback:l(function e(b,c){m(e),d.resolve(b,a.info=c)})})*/
                        ...
                        toData: function(a) {
                            for (var b = this.items, d = b.length; d--;) if (b[d].file && k(b[d].blob)) return this.parent.apply(this, arguments);
